...
export const parseSessionHolder = (session: Session): SessionHolder => {
  const cookieValue = decrypt<SessionHolder&{
    date: string
...
export const getOpenIdConfig = async (): Promise<OpenIdConfiguration> => {
  ...
  return fetch(uri)
      .catch(e => {
        throw new Error('Unable to fetch OIDC discovery on ' + uri + ': ' + e.toString())
      })
      .then(response => {
        if(response.status != 200) {
          throw new Error('Unexpected status code ' + response.status + ' when fetching openid config')
        }
...
export const exchangeToken = async (
  ...
  return await fetch(uri).then(response => {
    if (response.status != 200) {
      throw new Error('Unexpected status code ' + response.status + ' when refreshing token, body: ' + response.text())
    }
    return response.json();
  });
};
...
export const refreshToken = async (
  ...
  return fetch(uri).
    then(async response => {
      if(response.status != 200) {
        throw new Error('Unexpected status code ' + response.status + ' when refreshing token, body: ' + await response.text())
      }
...
export const triggerAuthenticationFlow = (url: URL, openIdConfiguration: OpenIdConfiguration, redirectTo: string) => {
  ...
  redirectUrl.searchParams.set('redirect_uri', `${url.origin}${AUTH_CALLBACK_ROUTE}`);
