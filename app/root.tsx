...
import { errorsMap, logger } from "~/src/utils/api.server";
import {
  AUTH_CALLBACK_ROUTE,
  commitSession,
  COOKIE_NAME,
  ...
  if (!cookie) {
    ...
    return handleResponse(
      await withSession(request, async () => {
        ...
        if (decryptedCookie.access_token) {
          console.info("Ok, got authenticated request");
        } else {
          console.info("no authenticated request");
        }
